...
#define MSG_LEN 8000	// the maximum length of the recieved message 
...
int main(void)
{
    ...
    char* message = malloc(sizeof(char)*MSG_LEN);
	long int filesize;
    while(1) {  // main accept() loop
		...
		if(recv(new_fd, message, MSG_LEN, 0) != -1){
			...
			if(!(_file)){
				...
				sprintf(message, "HTTP/1.0 200 Document Follows\nContent-Length: %ld\n\n", filesize);
				if (send(new_fd, message, strlen(message), 0) == -1)
					perror("send");
				fseek(_file, 0, SEEK_SET);
				while( filesize>0 ){
					if( filesize>MSG_LEN ){
						fread(message, sizeof(char), MSG_LEN, _file);
						filesize -= MSG_LEN;
					}
					else{
						fread(message, sizeof(char), filesize, _file);
						filesize = 0;
					}
					if (send(new_fd, message, strlen(message), 0) == -1)
						perror("send");
